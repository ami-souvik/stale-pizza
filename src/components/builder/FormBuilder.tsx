...
import PreviewCase from "../common/PreviewCase";
...
const innerStyles: { [Key in TCardState['type']]?: string } = {
    idle: 'hover:border-blue-300 cursor-grab',
...
function SidebarField({ field }: { field: Field }) {
    ...
    return (
        <div
            ref={ref}
            className={`p-3 bg-white border rounded-lg shadow-sm flex items-center space-x-3 cursor-grab hover:shadow-md transition-all ${state.type === 'is-dragging' ? 'opacity-50' : ''}`}
        >
...
function CanvasField({ 
    ...
    return (
        <div
            ...
            onClick={onSelect}
            className={cn(
                "relative group p-4 border-2 rounded-lg transition-all mb-3 bg-white",
...
export default function FormBuilder({ object: initObject }: { object: { appId: string | number, id: number, fields: Field[] } }) {
    ...
    const availableFields = useMemo(() => {
        const usedIds = new Set(formFields.map(f => f.id));
        return initObject.fields.filter(f => !usedIds.has(f.id));
    ...
    const handleSave = () => {
        ...
        const missingRequired = requiredFields.filter(f => !formFieldIds.has(f.id));
        ...
        // Save logic (mock)
        console.log('Saving form layout:', formFields.map(f => f.id));
        success('Form saved successfully');
    };

    return (
        <div ref={scrollableRef} className="h-full flex flex-col bg-gray-50">
            <div className="bg-white border-b px-6 py-3 flex justify-between items-center shadow-sm z-10">
                <div>
                    <h1 className="text-lg font-semibold text-gray-900">Form Layout</h1>
                    <p className="text-xs text-gray-500">Drag fields from the sidebar to build your form</p>
                </div>
            ...
            <div className="flex-1 overflow-hidden grid grid-cols-[300px_1fr_300px]">
                ...
                <div className="bg-white border-r border-gray-200 overflow-y-auto p-4">
                    <h3 className="font-semibold text-sm text-gray-900 mb-4">Available Fields</h3>
                ...
                {/* Canvas */}
                <div className="bg-gray-100 overflow-y-auto p-8 relative">
                    <div 
                        ref={mainRef}
                        className="max-w-2xl mx-auto bg-white rounded-xl shadow-sm min-h-[400px] p-8 border border-gray-200 relative"
                    >
                ...
                <div className="bg-white border-l border-gray-200 overflow-y-auto p-4">
